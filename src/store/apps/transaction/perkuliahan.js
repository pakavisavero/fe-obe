...
export const fetchDataHistoryKBM = createAsyncThunk(
  "perkuliahan/fetchDataHistoryKBM",
  async (params, { rejectWithValue }) => {
    try {
      const response = await axios.get("history-kbm", {
        params,
        paramsSerializer: (params) => qs.stringify(params),
      });
      return response.data;
...
export const addPerkuliahan = createAsyncThunk(
  "perkuliahan/addPerkuliahan",
...
export const updatePerkuliahan = createAsyncThunk(
  "perkuliahan/updatePerkuliahan",
  async (data, { getState, dispatch }) => {
    const config = {
      method: "put",
      url: "perkuliahan",
...
export const perkuliahansSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchDataHistoryKBM.pending, (state) => {
      state.loading = true;
    });
    builder.addCase(fetchDataHistoryKBM.fulfilled, (state, action) => {
      state.loading = false;
      if (action.payload.code !== 200) {
        state.error = action.payload.message;
      } else {
    ...
    builder.addCase(fetchDataHistoryKBM.rejected, (state, action) => {
      state.loading = false;
      state.error = action.payload.message;
    });

...
const FormDataWithChild = ({
  ...
  const { data: dataImpact, loading: loadingImpact } = useSelector(state => state.masterImpact)
  ...
  const handleAdd = data => {
    // const findImpact = dataImpact.find(item => item.id === data.impact_id)
    // data.impact_desc = findImpact?.impact_desc ?? ''
  ...
  return (
    <>
      <Grid container spacing={6} component={'form'} onSubmit={handleSubmit(defaultSubmit)} methods={methods}>
        <Grid item xs={12}>
          <Breadcrumbs data={dataBreadcrumbs} withDocStatus={withDocStatus} value={docValue} />
        </Grid>
        <Grid item xs={12}>
          <AddActions
            loading={loading}
      ...
      <SidebarChild
        name={titleChild}
        schema={yupSchemaChild}
        ContentForm={ContentFormChild}
        store={store}

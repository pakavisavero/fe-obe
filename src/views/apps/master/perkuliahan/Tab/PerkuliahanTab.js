...
import {
    ...
    uploadCPMK,
    clearResponse
} from 'src/store/apps/master/perkuliahan'
...
const PerkuliahanTab = ({ watch, control, errors, isEdit, setValue, setValueData }) => {
    ...
    const { data: dataDosen, loading: loadingDosen } = useSelector(state => state.user)
    const { data: dataTahunAjaran, loading: loadingTahunAjaran } = useSelector(state => state.tahunAjaran)
    const { data: dataMataKuliah, loading: loadingMataKuiah } = useSelector(state => state.mataKuliah)
    ...
    const fields = [
        ...
        },
        {
            ...
            changeSearch: SearchhandleFilterAutoComplete('mata_kuliah', fetchDataMataKuliah),
        ...
        },
        {
            ...
            xs: 12,
            md: 4,
            data: _.sortBy(watch('dosen1_id') ? dataDosen.filter(
                item => item.id != watch('dosen1_id')) : [], ['full_name']),
            loading: loadingDosen,
            optLabel: 'full_name',
            changeSearch: SearchhandleFilterAutoComplete('full_name', fetchDataDosen),
        ...
        },
        {
            ...
            xs: 12,
            md: 4,
            data: _.sortBy(watch('dosen2_id') ? dataDosen.filter(
                item => item.id != watch('dosen1_id') && item.id != watch('dosen2_id')) : [], ['full_name']),
            loading: loadingDosen,
            optLabel: 'full_name',
            changeSearch: SearchhandleFilterAutoComplete('full_name', fetchDataDosen),
        ...
        },
        {
            ...
            onChange: handleChangeAutoComplete('tahun_ajaran_id_name', 'tahun_ajaran'),
        },
        {
            name: 'semester',
            type: 'select',
        ...
        <>
            {isEdit &&
                <>
                    <CardContent>
                        <Grid container>
                            <ImportDialog callback={callbackImportDPNA} openDialog={open.dpna} handleClose={handleClose('dpna')} />
                            <ImportDialogCPMK callback={callbackImportCPMK} openDialog={open.cpmk} handleClose={handleClose('cpmk')} />

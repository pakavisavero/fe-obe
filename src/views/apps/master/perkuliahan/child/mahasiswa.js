...
function Mahasiswa({ control, store, watch }) {
    ...
    const defaultColumns = [
        ...
        },
        {
            ...
            minWidth: 350,
            renderCell: ({ row }) => (
                <Typography variant='body2'>{`${row.full_name.toUpperCase() || '-'}`}</Typography>
            )
        },
        {
        ...
        },
        {
            ...
            minWidth: 280,
            renderCell: ({ row }) => (
                <Typography variant='body2'>{`${row.doswal?.full_name || '-'}`}</Typography>
            )
        },
    ]
                ...
                renderCell: ({ row }) => {
                    return (
                        <Typography variant='body2' sx={{
                            ...
                            {`
                                ${row.raport?.nilai_pokok.nilai_akhir.toFixed(2) || '-'}
                            `}
                        </Typography>
                    )
                }

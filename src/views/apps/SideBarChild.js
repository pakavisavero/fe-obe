...
const SidebarChild = props => {
  // ** Props
  const { store, name, schema, ContentForm, onCancel, onAdd, onUpdate } = props
  const { isModelOpen, selectedChild } = store
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: 5 }}>
        <form method={methods}>
          {<ContentForm control={control} errors={errors} watch={watch} setValue={setValue} />}
